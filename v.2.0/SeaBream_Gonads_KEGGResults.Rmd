...
cnetplot(kegg, 
  ...
  scale_color_gradient2(name = 'log2FC', 
                        ...
                        high = 'tomato4')
```
*This figure presents the results of a KEGG enrichment analysis performed with the clusterProfiler package. It only shows significant KEGG pathways and ignores the others*
...
ggplot(kegg.terms,
  ...
  ggthemes::theme_few()
```
*This figure presents the results of a KEGG enrichment analysis performed with the gage package. It shows all KEGG pathways, even the non-significant ones. Results should be very similar to what is presented before, but differences may occur due to differences in KEGG databases.*
...
a + b + patchwork::guide_area() + 
  patchwork::plot_layout(ncol = 1, 
                        ...
                        guides  = "collect")
```
*In this figure, we see the percentage of Sparus aurata genes finding at least one homolog/ortholog in other species. Below, I highlighted some genes of interest and check if they are present in each species where homologs were detected.*
...
```
*This figure presents the contributions of each KEGG-supported species to the annotations of Sparus aurata genes. It differs from the previous graph because we only retained homologs with high quality and removed low-quality ones. As a result, Seriola dumerili becomes the best species to map Sparus aurata genes onto.*
...
```
*These figures show how the best homologous genes to the Sparus aurata genes we want to annotate are distributed across KEGG-supported organisms. Again, Seriola dumerili comes up on top.*
...
ggplot(One_homolog, 
  ...
  theme(legend.position = "none")
```
*This figure shows the distribution of the best homologous genes to Sparus aurata genes which also carry a KEGG annotations. Before we only picked the KEGG-supported organisms which had high-quality homologs to Sparus aurata genes and sorted them so that the best species would show. But not all the genes actually carry a KEGG annotation. Here we see the part of Sparus aurata genes which cannot be annotated AT ALL.*
 
 
 
## 5. Final KEGG annotations
...
### 5.a. Strategy 1: Multi-species KEGG analysis
...
summary(gsea.custom) %>% dplyr::select(-Description)
```
*These results show the KEGG categories which are returned when annotating Sparus aurata genes with multiple species homologs. The results are not more precise than what has been observed when mapping Sparus aurata genes onto Danio rerio or Oryzias latipes.*
...
#         kegg.dir    = kegg.dir)
```
*This figure shows the results of a KEGG enrichment analysis performed with the gage package on Seriola dumerili.*
...
## 6. Guilty by association {.tabset}
...
### 6.b. Experimental co-expression
